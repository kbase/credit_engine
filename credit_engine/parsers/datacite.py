...
DATACITE_SAMPLE_DATA_DIR = "sample_data/datacite"
...
def retrieve_doi_list(
    ...
    save_dir: Optional[Union[Path, str]] = DATACITE_SAMPLE_DATA_DIR,
    ...
    :param save_dir: the directory to save files to, defaults to DATACITE_SAMPLE_DATA_DIR
    ...
    if save_files:
        ...
        if not save_dir:
            # use the sample dir
            save_dir = DATACITE_SAMPLE_DATA_DIR
    ...
    for doi in cleaned_doi_list:
        ...
        if save_files:
            ...
            doi_file = os.path.join(save_dir, f"{doi_to_file_name(doi)}.json")
            try:
